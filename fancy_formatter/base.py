...
class FormatResult:
    ...
    @classmethod
    def from_subprocess_exception(
        cls: "FormatResult", e: subprocess.CalledProcessError
    ) -> "FormatResult":
        result = FormatResult()
        result.Code = EFormatResult.Fatal
        stderr = e.stderr
    ...
    def from_subprocess_result(
        cls: "FormatResult", p: subprocess.CompletedProcess
    ) -> "FormatResult":
        stdout = p.stdout
...
def execute_with_pipe(args: List[str], text: str) -> FormatResult:
    ...
    try:
        ...
        if e.filename:
            return FormatResult.fatal_error(
                f"{e.strerror} : {e.filename} {e.filename2}"
            )
        else:
